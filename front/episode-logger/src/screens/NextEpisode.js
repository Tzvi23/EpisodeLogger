...
const NextEpisode = (props) => {
  ...
  const getUserDataCall = () => {
    ...
    fetch("/getUserData", body).then((response) =>
    ...
    <div>
      ...
      >
        {Object.keys(props.userData).map((series, i) => {
          if (
            props.userData[series].visible === true &&
            props.userData[series].nextEpisode !== null
          )
            return (
              <NextEpisodeCard
                seriesData={props.userData[series]}
